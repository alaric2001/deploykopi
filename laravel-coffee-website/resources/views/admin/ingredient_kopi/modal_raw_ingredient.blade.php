<div id="myModal" class="hidden">
    <div class="modal-content w-[440px] bg-white p-8 rounded-lg shadow-md m-auto h-auto">
        <div class="flex justify-between items-center">
            <h1 class="text-lg font-semibold mb-4">Tambah Ingredient</h1>
            <button id="closeModalButton" class="p-2">
                <h1 class="text-lg font-semibold mb-4">x</h1>
            </button>
        </div>
        
        <form action="/add-rawingredient" method="POST">
            @csrf
            <div class="mb-4">
                <label for="jenis" class="block text-sm font-medium text-gray-700">Nama Ingredient</label>
            ...
            <div class="mb-4">
                <label for="stokingredient" class="block text-sm font-medium text-gray-700">Stok Ingredient</label>
                <input type="number" name="stok" id="stokingredient" class="mt-1 focus:ring-secondary focus:border-secondary w-full shadow-sm sm:text-sm border-gray-300 rounded-md" required>
            </div>
...
<div id="editModal" class="hidden">
    <div class="modal-content w-[440px] bg-white p-8 rounded-lg shadow-md m-auto h-auto">
        <div class="flex justify-between items-center">
            <h1 class="text-lg font-semibold mb-4">Edit Ingredient</h1>
        ...
        <form id="editIngredientForm" method="POST">
            ...
            @method('PUT')
            <div class="mb-4">
                <label for="edit_jenis" class="block text-sm font-medium text-gray-700">Nama Ingredient</label>
            ...
            <div class="mb-4">
                <label for="editstokingredient" class="block text-sm font-medium text-gray-700">Stok Ingredient</label>
                <input type="number" name="stok" id="editstokingredient" class="mt-1 focus:ring-secondary focus:border-secondary w-full shadow-sm sm:text-sm border-gray-300 rounded-md" required>
            </div>
...
<script>
    ...
    document.querySelectorAll('.edit-button').forEach(button => {
        button.addEventListener('click', function(event) {
            ...
            const stok = this.getAttribute('data-stokingredient');
            // const kopiId = this.getAttribute('data-kopi');
            document.getElementById('edit_jenis').value = nama;
            document.getElementById('editstokingredient').value = stok;
            document.getElementById('editIngredientForm').setAttribute('action', '/edit-rawingredient/' + id);
